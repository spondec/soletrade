...
class TradeLoop
{
    ...
    protected function getCandlesBetween(int $endDate): Collection
    {
        ...
        if (!$candles?->first()) {
            throw new PrintableException("Not enough price data found for {$symbol->exchange()::name()}-$symbol->symbol-$symbol->interval. ".'Please use a different interval or exchange.');
        }
